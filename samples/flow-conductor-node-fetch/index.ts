 ...
 *
 * This example demonstrates how to use flow-conductor with the node-fetch adapter
...
import type { NodeFetchRequestConfig } from "@flow-conductor/adapter-node-fetch";

// Create a node-fetch adapter instance
...
async function example1_SimpleChain() {
  ...
  try {
    const result = await begin<Post, Response, NodeFetchRequestConfig>(
      {
        config: {
...
async function example2_MultiStepChain() {
  ...
  try {
    const result = await begin<User, Response, NodeFetchRequestConfig>(
      {
        config: {
...
async function example4_ErrorHandling() {
  ...
  try {
    await begin<Post, Response, NodeFetchRequestConfig>(
      {
        config: {
 ...
 */
async function main() {
  console.log("🚀 Flow-Conductor Node-Fetch Adapter Examples");
  console.log(
